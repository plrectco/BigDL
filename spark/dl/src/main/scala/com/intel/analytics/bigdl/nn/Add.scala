...
class Add[T: ClassTag](val inputSize: Int
  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T]): Unit = {
    ...
    if (gradBias.size(1) == 1) {
      gradBias(1) = gradBias(1).add(ev.times(ev.fromType(scaleB), gradOutput.sum()))
    } else {
      if (input.isSameSizeAs(bias)) {
        ...
        gradBias.view(gradBias.size().product).addmv(ev.fromType(scaleB), gradOutputLocal.t(), ones)
      }
    }
  }
